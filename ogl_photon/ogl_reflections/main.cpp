...
public:
	PObject() { init(1024, 4); }
	PObject(GLuint count) { init(count, 4); }
	PObject(GLuint count, GLuint d) { init(count, d); }
...
private:
	...
	GLuint vsize_g = sizeof(Voxel) * 256 * 256 * 256;
	GLuint vrsize_g = sizeof(VoxelRawPhoton) * 512 * 512 * 32;
	GLuint tsize_g = sizeof(ThashPhoton) * 512 * 512 * 64;
	GLuint subgridc_g = sizeof(GLuint) * 256 * 256 * 256 * 8;
...
public:
	...
	void setDepth(GLuint count, GLuint d) {
		...
		GLuint vrsize = sizeof(VoxelRawPhoton) * count / 2;
		GLuint vsize = sizeof(Voxel) * size_r * size_r * 32;
		GLuint tsize = sizeof(ThashPhoton) * count;
		GLuint subgridc = sizeof(GLuint) * size_r * size_r * 32 * 8;
...
private:
	void init(GLuint count, GLuint depth) {
		...
		{
			GLuint compShader = loadShader("./photon_tracer/intersection.comp", GL_COMPUTE_SHADER);
		...
		{
			GLuint compShader = loadShader("./photon_tracer/begin.comp", GL_COMPUTE_SHADER);
		...
		{
			GLuint compShader = loadShader("./photon_tracer/close.comp", GL_COMPUTE_SHADER);
		...
		{
			GLuint compShader = loadShader("./photon_tracer/generator.comp", GL_COMPUTE_SHADER);
		...
		{
			GLuint compShader = loadShader("./photon_builder/fix.comp", GL_COMPUTE_SHADER);
			voxelizerFixProgram = glCreateProgram();
			glAttachShader(voxelizerFixProgram, compShader);
			glBindFragDataLocation(voxelizerFixProgram, 0, "outColor");
			glLinkProgram(voxelizerFixProgram);
		...
		{
			GLuint compShader = loadShader("./photon_builder/filler.comp", GL_COMPUTE_SHADER);
			voxelizerFillerProgram = glCreateProgram();
			glAttachShader(voxelizerFillerProgram, compShader);
			glBindFragDataLocation(voxelizerFillerProgram, 0, "outColor");
			glLinkProgram(voxelizerFillerProgram);
		...
		{
			GLuint vertexShader = loadShader("./photon_tracer/debug.vert", GL_VERTEX_SHADER);
			GLuint fragmentShader = loadShader("./photon_tracer/debug.frag", GL_FRAGMENT_SHADER);
...
public:
	void bindOctree() {
		...
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, subgrid);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, tspace);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 11, vrspace);
		glBindBufferBase(GL_ATOMIC_COUNTER_BUFFER, 2, dcounter);
	}
	...
	void intersection(TObject &obj, glm::mat4 trans) {
		...
		glUniform1ui(glGetUniformLocation(intersectionProgram, "photonCount"), photonCount);
		glDispatchCompute(tiled(photonCount, 1024) / 1024, 1, 1);
		glMemoryBarrier(GL_ALL_BARRIER_BITS);
	}
	...
	void generate(glm::vec3 view) {
		...
		bindPhoton();
		glUniform3fv(glGetUniformLocation(cameraProgram, "center"), 1, glm::value_ptr(view));
		glUniform1f(glGetUniformLocation(cameraProgram, "radius"), 0);
		glUniform1f(glGetUniformLocation(cameraProgram, "time"), clock());

		glUniform1ui(glGetUniformLocation(cameraProgram, "photonCount"), photonCount);
		glDispatchCompute(tiled(photonCount, 1024) / 1024, 1, 1);
		glMemoryBarrier(GL_ALL_BARRIER_BITS);
	}
	...
	void begin() {
		...
		glUniform1ui(glGetUniformLocation(beginProgram, "photonCount"), photonCount);
		glDispatchCompute(tiled(photonCount, 1024) / 1024, 1, 1);
		glMemoryBarrier(GL_ALL_BARRIER_BITS);
	}
	...
	void close() {
		...
		glUniform1ui(glGetUniformLocation(closeProgram, "photonCount"), photonCount);
		glDispatchCompute(tiled(photonCount, 1024) / 1024, 1, 1);
		glMemoryBarrier(GL_ALL_BARRIER_BITS);
	}
	...
	void render() {
		...
		glUniform1ui(glGetUniformLocation(renderProgram, "photonCount"), photonCount);
...
private:
	...
	void init() {
		...
		{
			GLuint compShader = loadShader("./photon_tracer/testmat.comp", GL_COMPUTE_SHADER);
			matProgramPhoton = glCreateProgram();
			glAttachShader(matProgramPhoton, compShader);
			glBindFragDataLocation(matProgramPhoton, 0, "outColor");
			glLinkProgram(matProgramPhoton);
			glUseProgram(matProgramPhoton);
		}
	}
public:
	TestMat() {
	...
	void shadePhoton(PObject &rays, GLfloat reflectionRate) {
		glUseProgram(matProgramPhoton);

		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, tex);
		glUniform1i(glGetUniformLocation(matProgramPhoton, "tex"), 0);

		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, btex);
		glUniform1i(glGetUniformLocation(matProgramPhoton, "bump"), 1);

		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, stex);
		glUniform1i(glGetUniformLocation(matProgramPhoton, "spec"), 2);
		...
		glUniform1f(glGetUniformLocation(matProgramPhoton, "time"), clock());
		glUniform1ui(glGetUniformLocation(matProgramPhoton, "materialID"), materialID);
		glUniform1f(glGetUniformLocation(matProgramPhoton, "reflectionRate"), reflectionRate);
		glUniform3fv(glGetUniformLocation(matProgramPhoton, "light"), 1, glm::value_ptr(glm::vec3(9.0f, 200.0f, 9.0f)));
		glUniform1ui(glGetUniformLocation(matProgramPhoton, "currentDepth"), rays.maxDepth - 1);
		glUniform1ui(glGetUniformLocation(matProgramPhoton, "maxDepth"), rays.maxDepth);

		glUniform1ui(glGetUniformLocation(matProgramPhoton, "photonCount"), rays.photonCount);
		glDispatchCompute(tiled(rays.photonCount, 1024) / 1024, 1, 1);
		glMemoryBarrier(GL_ALL_BARRIER_BITS);
	}

	void shade(PObject &photons, RObject &rays, GLfloat reflectionRate) {
		glUseProgram(matProgram);
		...
		glUniform1ui(glGetUniformLocation(matProgram, "currentDepth"), photons.maxDepth-1);
		glUniform1ui(glGetUniformLocation(matProgram, "maxDepth"), photons.maxDepth);
		glUniform1ui(glGetUniformLocation(matProgram, "photonCount"), photons.photonCount);
		glDispatchCompute(tiled(width, 32) / 32, tiled(height, 32) / 32, 1);
		glMemoryBarrier(GL_ALL_BARRIER_BITS);
	}
};
